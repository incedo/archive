...
"$TOFU_BIN" plan -refresh=false -lock=false -input=false -var-file=dev.tfvars >/tmp/archive-tofu-auth-check.txt 2>&1 || true

if rg -q "No valid credential sources found" /tmp/archive-tofu-auth-check.txt; then
  cat /tmp/archive-tofu-auth-check.txt
  exit 1
fi
