...
"$TOFU_BIN" plan -refresh=false -lock=false -input=false -var-file=dev.tfvars >/tmp/archive-tofu-scaleway-auth-check.txt 2>&1 || true

if rg -q "authentication|unauthorized|invalid credential|forbidden" /tmp/archive-tofu-scaleway-auth-check.txt; then
  cat /tmp/archive-tofu-scaleway-auth-check.txt
  exit 1
fi
