...
if ! command -v "$TOFU_BIN" >/dev/null 2>&1; then
  ...
  echo "Install OpenTofu or set TOFU_BIN to the correct executable path." >&2
  exit 1
fi

if [ ! -f "$TFVARS_FILE" ]; then
  echo "Missing tfvars file: $TFVARS_FILE" >&2
  echo "Copy dev.tfvars.example to dev.tfvars and fill in the real Scaleway values." >&2
  exit 1
fi
...
"$TOFU_BIN" init
"$TOFU_BIN" plan -var-file="$TFVARS_FILE"
